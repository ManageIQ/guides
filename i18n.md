...
First, extract model names and attributes by running the following:

```
$ RAILS_ENV=i18n bundle exec rake locale:store_model_attributes
```

Next, extract strings from `en.yml` and various other yaml files. From the root of ManageIQ checkout, run the following:

```
$ bundle exec rake locale:store_dictionary_strings
$ bundle exec rake locale:extract_yaml_strings
```
